...
import { listToPrograms } from '../services';
...
export default function ToConsultaProgramasPage() {
    ...
    const handleCreateProgram = () => {
        if (!selectedPatient) {
            alert('Por favor, selecione um cliente antes de criar um programa de TO.');
            return;
        }

        navigate(`${toRoutes.create}?patientId=${selectedPatient.id}&patientName=${encodeURIComponent(selectedPatient.name)}`);
    };
    ...
    const handleNewSession = (programId: string) => {
        ...
        navigate(toRoutes.newSession(programId, selectedPatient.id));
    };
