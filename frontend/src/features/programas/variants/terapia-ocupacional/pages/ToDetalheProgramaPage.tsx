...
import ToProgramActionBar from '../components/ToProgramActionBar';
import ToSummaryCard from '../components/ToSummaryCard';
import ToLastSessionPreview from '../components/ToLastSessionPreview';
import ToSessionsList from '../components/ToSessionsList';
import type { ToActivitySummary } from '../types';

// Tipo específico para TO com campos adicionais
type ToProgramDetail = {
    id: string;
    name?: string | null;
    patientId: string;
    patientName: string;
    ...
    stimuli: {
        ...
        order: number;
        label: string;
        description?: string | null; // TO usa descrição por estímulo
        active: boolean;
    }[];
    criteria?: string | null; // TO não usa, mas mantém por compatibilidade
    notes?: string | null;
...
export default function ToDetalheProgramaPage() {
    ...
    const [program, setProgram] = useState<ToProgramDetail | null>(null);
                ...
                <div data-print-meta-header data-print-only className="hidden">
                    ...
                    >
                        ...
                        <div className="text-right text-sm text-muted-foreground">
                            ...
                            <p className="font-medium">Programa de Terapia Ocupacional</p>
                        </div>
                    </div>
                </div>
                ...
                <div data-print-content className="flex flex-col">
                    <div className="min-h-screen mt-4 pb-28 sm:p-0 rounded-lg">
                        <div className="md:max-w-none mx-auto md:mx-0 lg:mx-4 space-y-4">
                            ...
                            {/* Nível Atual de Desempenho (específico de TO) */}
                            <ToCurrentPerformanceSection currentPerformanceLevel={program.currentPerformanceLevel} />
                            ...
                            {lastSession && (lastSession as any).activitiesSummary && (
                                <ToLastSessionPreview
                                    ...
                                    activitiesSummary={(lastSession as any).activitiesSummary as ToActivitySummary[]}
                                />
                            )}
