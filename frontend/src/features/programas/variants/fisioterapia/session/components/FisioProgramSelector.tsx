// Componente de seleção de programa Fisio para sessão
// Usa a mesma UI do ProgramSelector mas com lógica adaptada para TO
...
interface ToProgramSelectorProps {
...
export default function ToProgramSelector({
    patient,
    selected,
    onSelect,
    onClear,
    autoOpenIfEmpty = false,
}: ToProgramSelectorProps) {
    ...
    useEffect(() => {
        ...
        const timeoutId = setTimeout(async () => {
            setIsLoading(true);
            try {
                // Usar o serviço de Fisio ao invés de Fono
                const { listFisioPrograms } = await import('../../services/api');
                const result = await listFisioPrograms({
                ...
                console.error('Erro ao buscar programas TO:', error);
                setPrograms([]);
            } finally {
                setIsLoading(false);
            }
                    ...
                    {selected ? (
                        <div className="flex items-center gap-3 p-2 sm:p-3 bg-muted rounded-[5px]">
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="font-regular text-base truncate">{selected.title}</p>
                ...
                >
                    <div className="bg-background w-full max-w-2xl h-[85vh] md:h-[80vh] rounded-t-lg md:rounded-lg shadow-lg animate-in slide-in-from-bottom md:fade-in duration-300 flex flex-col">
                        <div className="flex items-center justify-between p-4 sm:p-6 border-b">
                            <h2 className="text-lg font-regular" style={{fontFamily: "sora"}}>
                                Selecionar Programa / Objetivo
                            </h2>
                                ...
                                <div className="space-y-2 max-h-[60vh] overflow-y-auto">
                                    {isLoading ? (
                                        ...
                                        <div className="text-center py-8 text-muted-foreground">
                                            {searchQuery
                                                ? 'Nenhum programa encontrado com os critérios de busca'
                                                : 'Nenhum programa ativo encontrado para este cliente'}
                                        </div>
                                    ) : (
