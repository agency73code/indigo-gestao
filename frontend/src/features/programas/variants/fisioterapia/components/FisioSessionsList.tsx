/**
 * ToSessionsList
 * Versão adaptada de SessionsList para Fisioterapia
 * 
...
type FisioStatusKind = 'desempenhou' | 'desempenhou_com_ajuda' | 'nao_desempenhou';
...
const FISIO_STATUS_CONFIG: Record<FisioStatusKind, StatusBadgeConfig> = {
    desempenhou: {
        icon: CheckCircle,
...
function StatusBadge({
    ...
    summary: { kind: FisioStatusKind; count: number; total: number };
    testId: string;
    tooltipId: string;
}) {
    const { kind, count, total } = summary;
    const config = FISIO_STATUS_CONFIG[kind];
