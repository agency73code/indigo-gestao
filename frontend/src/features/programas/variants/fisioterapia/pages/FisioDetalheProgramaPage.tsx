...
import ToSummaryCard from '../components/FisioSummaryCard';
import ToLastSessionPreview from '../components/FisioLastSessionPreview';
import ToSessionsList from '../components/FisioSessionsList';
import type { FisioActivitySummary } from '../types';

// Tipo específico para Fisio com campos adicionais
type FisioProgramDetail = {
    id: string;
    name?: string | null;
    patientId: string;
    patientName: string;
    ...
    currentPerformanceLevel?: string | null; // CAMPO ESPECÍFICO DE TO
    stimuliApplicationDescription?: string | null;
    stimuli: {
        id: string;
        order: number;
        label: string;
        description?: string | null; // Fisio usa descrição por estímulo
        active: boolean;
    }[];
    criteria?: string | null; // Fisio não usa, mas mantém por compatibilidade
    notes?: string | null;
...
export default function ToDetalheProgramaPage() {
    const { programaId } = useParams<{ programaId: string }>();
    const { setPageTitle, setHeaderActions } = usePageTitle();

    const [program, setProgram] = useState<FisioProgramDetail | null>(null);
    ...
    const handlePrint = usePrint({
        ...
        documentTitle: `programa_to_${program?.name || program?.id || 'detalhado'}`,
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            if (programaId === 'mock-to-001') {
                ...
                setProgram(mockData as FisioProgramDetail);
                ...
                setProgram(programData as FisioProgramDetail);
            ...
            console.error('Erro ao carregar dados do programa de Fisio:', err);
                ...
                <div data-print-meta-header data-print-only className="hidden">
                    ...
                    >
                        ...
                        <div className="text-right text-sm text-muted-foreground">
                            ...
                            <p className="font-medium">Programa de Fisioterapia</p>
                        </div>
                    </div>
                </div>
                ...
                <div data-print-content className="flex flex-col">
                    <div className="min-h-screen mt-4 pb-28 sm:p-0 rounded-lg">
                        <div className="md:max-w-none mx-auto md:mx-0 lg:mx-4 space-y-4">
                            ...
                            {/* Objetivo Geral */}
                            <ToGoalSection program={program} />

                            {/* Objetivo Específico (Atividades) */}
                            <ToStimuliSection program={program} />

                            {/* Observações do Terapeuta */}
                            <ToNotesSection notes={program.notes || ''} readOnly={true} />

                            {/* Lista de sessões recentes */}
                            {sessions.length > 0 && (
                                <ToSessionsList sessions={sessions} program={program} />
                            )}
                            ...
                            {lastSession && (lastSession as any).activitiesSummary && (
                                <ToLastSessionPreview
                                    ...
                                    activitiesSummary={(lastSession as any).activitiesSummary as FisioActivitySummary[]}
                                />
                            )}
                        ...
                        <div className="mt-4">
                            <ToProgramActionBar program={program} />
                        </div>
                    </div>
                </div>
            </div>
