...
import { listMusiPrograms } from '../services';
import { musiRoutes } from '../config';

export default function MusiConsultaProgramasPage() {
    const [searchParams, setSearchParams] = useSearchParams();
    const navigate = useNavigate();
    const { setPageTitle } = usePageTitle();

    useEffect(() => {
        setPageTitle('Consultar Programas & Objetivos - Musicoterapia');
    }, [setPageTitle]);
    ...
    const handleCreateProgram = () => {
        if (!selectedPatient) {
            alert('Por favor, selecione um cliente antes de criar um programa de Musicoterapia.');
            return;
        }

        navigate(`${musiRoutes.create}?patientId=${selectedPatient.id}&patientName=${encodeURIComponent(selectedPatient.name)}`);
    };

    const handleOpenProgram = (programId: string) => {
        navigate(musiRoutes.detail(programId));
    };

    const handleNewSession = (programId: string) => {
        if (!selectedPatient) return;
        navigate(musiRoutes.newSession(programId, selectedPatient.id));
    };
                ...
                <div className="space-y-4 max-w-full">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <Button
                            ...
                            title="Adicionar programa de Musicoterapia"
                        >
                    ...
                    <ProgramList
                        ...
                        onListPrograms={listMusiPrograms}
