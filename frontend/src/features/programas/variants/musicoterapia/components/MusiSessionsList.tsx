/**
 * MusiSessionsList
 * Versão adaptada de SessionsList para Musicoterapia
 */
...
interface MusiSessionsListProps {
...
function StatusBadge({
    ...
    summary: { kind: MusiStatusKind; count: number; total: number };
    testId: string;
    tooltipId: string;
}) {
    const { kind, count, total } = summary;
    const config = MUSI_STATUS_CONFIG[kind];
...
export default function MusiSessionsList({ sessions, program }: MusiSessionsListProps) {
    ...
    const handleViewSession = (session: SessionListItem) => {
        navigate(`/app/programas/musicoterapia/sessoes/${session.id}?pacienteId=${program.patientId}`, {
    ...
    const handleNewSession = () => {
        navigate(
            `/app/programas/musicoterapia/sessoes/nova?programaId=${program.id}&patientId=${program.patientId}`,
        );
    };

    const handleSeeAll = () => {
        navigate(`/app/programas/musicoterapia/sessoes/consultar?pacienteId=${program.patientId}`);
    };
