...
interface MusiListaSessoesProps {
    sessoes: Sessao[];
...
function calcMusiStatus(desempenhou: number, ajuda: number, naoDesempenhou: number): StatusKind {
...
export default function MusiListaSessoes({ sessoes, onVerDetalhes }: MusiListaSessoesProps) {
        ...
        <div className="grid gap-4 sm:gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2">
            <TooltipProvider>
                {sessoes.map((sessao) => {
                    ...
                    const statusKind = calcMusiStatus(desempenhou, ajuda, naoDesempenhou);
