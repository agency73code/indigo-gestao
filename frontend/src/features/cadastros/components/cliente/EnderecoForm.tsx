...
export default function EnderecoForm({ endereco, index, onUpdate, onRemove, errors }: EnderecoFormProps) {
        ...
        <div className="border rounded-lg p-4 space-y-4 relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-2 md:col-span-2">
                    <Label htmlFor={`logradouro-${index}`}>Logradouro {index === 0 ? '*' : ''}</Label>
                    <Input
                        id={`logradouro-${index}`}
                        value={endereco.logradouro || ''}
                        onChange={(e) => onUpdate(index, 'logradouro', e.target.value)}
                        placeholder="Rua, Avenida, etc."
                        className={errors[`enderecos.${index}.logradouro`] ? 'border-destructive' : ''}
                    />
                ...
                {/* Complemento */}
                <div className="space-y-2">
                    <Label htmlFor={`complemento-${index}`}>Complemento {index === 0 ? '*' : ''}</Label>
                    <Input
                        id={`complemento-${index}`}
                        value={endereco.complemento || ''}
                        onChange={(e) => onUpdate(index, 'complemento', e.target.value)}
                        placeholder="Apto, Casa, etc."
                        className={errors[`enderecos.${index}.complemento`] ? 'border-destructive' : ''}
                    />
