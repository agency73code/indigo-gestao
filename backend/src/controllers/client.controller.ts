...
export async function getClientById(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const contatosGerais = pagamento?.pagamento_contatos.filter((c) => c.categoria === 'geral') ?? [];
      const contatosAdv = pagamento?.pagamento_contatos.filter((c) => c.categoria === 'advogado') ?? [];

      const telsGerais = contatosGerais.filter((c) => c.tipo === 'telefone').map((c) => c.valor);
      const emailsGerais = contatosGerais.filter((c) => c.tipo === 'email').map((c) => c.valor);
