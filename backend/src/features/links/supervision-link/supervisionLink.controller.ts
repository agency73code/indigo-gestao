...
import {
    archiveSupervisionLinkService,
    createSupervisionLinkService,
    endSupervisionLinkService,
    getAllSupervisionLinksService,
    UpdateSupervisionLinkService,
} from './supervisionLink.service.js';
 ...
 */
export async function createSupervisionLinkController(
    req: Request,
    res: Response,
): Promise<Response> {
    try {
        ...
        const message =
            error instanceof Error
                ? error.message
                : 'Erro desconhecido ao criar vínculo de supervisão.';
 ...
 */
export async function getAllSupervisionLinksController(
    req: Request,
    res: Response,
): Promise<Response> {
    try {
        ...
        const message =
            error instanceof Error
                ? error.message
                : 'Erro desconhecido ao listar vínculos de supervisão.';
        return res.status(400).json({ message });
    }
}
 ...
 */
export async function UpdateSupervisionLinkController(
    req: Request,
    res: Response,
): Promise<Response> {
    try {
 ...
 */
export async function archiveSupervisionLinkController(
    req: Request,
    res: Response,
): Promise<Response> {
    try {
        ...
        if (!id) {
            return res
                .status(400)
                .json({ message: "O campo 'id' é obrigatório para arquivar um vínculo." });
        }
        ...
        const message =
            error instanceof Error
                ? error.message
                : 'Erro desconhecido ao arquivar vínculo de supervisão.';
        return res.status(400).json({ message });
    }
}
